% Gnossienne No. 3, Erik Satie (1890). Public domain (composer d. 1925).
% Notes taken verbatim from the Mutopia Project's public-domain edition
% (https://www.mutopiaproject.org/ftp/SatieE/Gnossienne/no_3/no_3.ly,
% engraved by Frédéric Duperray), reshaped into eroica's expected
% upMusic/downMusic voices. The original's cross-staff engraving trick
% (some notes visually drawn on the other staff) is dropped since it's a
% purely visual effect eroica's own score layout doesn't reproduce — the
% pitches themselves are unchanged. Barlines/bar numbers are hidden through
% most of the piece and restored near the end, matching Satie's own
% famously barline-less original notation.
...
upMusic = {
  ...
  \time 4/4
  <<
    \new Voice { \voiceOne \highNotesTrebleStaff }
    \new Voice { \voiceTwo \midNotesTrebleStaff }
  >>
}
...
downMusic = {
  ...
  \time 4/4
  <<
    \new Voice { \voiceTwo \midNotesBassStaff }
    \new Voice { \voiceFour \lowNotesBassStaff }
  >>
}
